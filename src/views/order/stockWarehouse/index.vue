      ...
      <div class="search" v-show="showSearch">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
          ...
          <!--<el-form-item label="订单类型" prop="orderType">
            <el-input v-model="queryParams.orderType" placeholder="订单类型" clearable style="width: 240px" @keyup.enter="handleQuery" />
          </el-form-item>-->
          <el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="View" :disabled="single" @click="handleView()" v-hasPermi="['order:stockOrder:edit']">详情</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Edit" :disabled="multiple" @click="handleBatchSell()" :loading="buttonLoading"  v-hasPermi="['order:stockOrder:edit']">强制平仓</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="multiple" @click="handleLock()" v-hasPermi="['order:stockOrder:edit']">锁仓</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="multiple"  @click="handleUnLock()" v-hasPermi="['order:stockOrder:edit']">解仓</el-button>
          </el-col>
...
import { listStockOrder, getStockOrder, delStockOrder, addStockOrder, updateStockOrder, lockStockOrder, batchSellStockOrder } from '@/api/order/stockOrder';
...
const { stock_order_status_dic, stock_order_pending_status_dic, stock_order_trade_type_dic, stock_order_close_dict, member_account_type_dict, stock_order_query_type_dict, yes_or_no_dic } = toRefs<any>(proxy?.useDict('stock_order_status_dic', 'stock_order_pending_status_dic', 'stock_order_trade_type_dic', 'stock_order_close_dict', 'member_account_type_dict', 'stock_order_query_type_dict', 'yes_or_no_dic'));
...
const orderTypeMap = {1: "杠杆交易", 2: "大宗交易", 3: "新股交易", 4: "普通交易"};
...
const getList = async () => {
  ...
  queryParams.value.queryStatus = 1;
...
const handleBatchSell = async () => {
  ...
  await proxy?.$modal.confirm('是否确认强制平仓股票交易订单编号为"' + _ids + '"的数据项？').finally(() => buttonLoading.value = false);
  buttonLoading.value = true;
  const res = await batchSellStockOrder(_ids).finally(() => buttonLoading.value = false);
  proxy?.$modal.msgSuccess("操作成功，股票交易订单数量：" + res.data);
  await getList();
}

/** 锁仓按钮操作 */
const handleLock = async (row?: StockOrderVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认锁仓股票交易订单编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await lockStockOrder({ ids: _ids, lockStatus: 1 });
  proxy?.$modal.msgSuccess("锁仓成功");
  await getList();
}

/** 解仓按钮操作 */
const handleUnLock = async (row?: StockOrderVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认解仓股票交易订单编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await lockStockOrder({ ids: _ids, lockStatus: 0 });
