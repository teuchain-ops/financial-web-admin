<template>
  <div class="p-2">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['product:stockBlockTrade:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['product:stockBlockTrade:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['product:stockBlockTrade:remove']"
              >删除</el-button
            >
          </el-col>
          <!--          <el-col :span="1.5">-->
          <!--            <el-button type="danger" plain icon="Promotion" :disabled="multiple" @click="handleLock()" v-hasPermi="['product:stockBlockTrade:edit']"-->
          <!--            >锁仓</el-button-->
          <!--            >-->
          <!--          </el-col>-->
          <!--          <el-col :span="1.5">-->
          <!--            <el-button type="danger" plain icon="Promotion" :disabled="multiple" @click="handleUnLock()" v-hasPermi="['product:stockBlockTrade:edit']"-->
          <!--            >解仓</el-button-->
          <!--            >-->
          <!--          </el-col>-->
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['product:stockBlockTrade:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="stockBlockTradeList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['product:stockBlockTrade:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button
                ...
                v-hasPermi="['product:stockBlockTrade:remove']"
              ></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="50%" append-to-body>
      <el-form ref="stockBlockTradeFormRef" :model="form" :rules="rules" label-width="120px">
        <el-row :span="8">
          <el-form-item label="股票代码" prop="stockCode">
            <el-select
          ...
          <el-form-item label="当前价格" prop="price">
            <el-input v-model="form.price" placeholder="请输入当前价格" style="width: 240px" :disabled="true" />
            <el-button type="primary" size="small" icon="Refresh" title="刷新" @click="getPrice(form.stockCode, form.exchangeCode)" style="margin-left: 10px" :disabled="isLoading" />
          </el-form-item>
...
<script setup name="StockBlockTrade" lang="ts">
import {
  listStockBlockTrade,
  getStockBlockTrade,
  delStockBlockTrade,
  addStockBlockTrade,
  updateStockBlockTrade,
  lockStockBlock
} from '@/api/product/stockBlockTrade';
import { StockBlockTradeVO, StockBlockTradeQuery, StockBlockTradeForm } from '@/api/product/stockBlockTrade/types';
...
const stockBlockTradeList = ref<StockBlockTradeVO[]>([]);
...
const stockBlockTradeFormRef = ref<ElFormInstance>();
...
const initFormData: StockBlockTradeForm = {
  stockName: undefined,
  stockCode: undefined,
...
const data = reactive<PageData<StockBlockTradeForm, StockBlockTradeQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
...
const getList = async () => {
  loading.value = true;
  const res = await listStockBlockTrade(queryParams.value);
  stockBlockTradeList.value = res.rows;
...
const reset = () => {
  ...
  stockBlockTradeFormRef.value?.resetFields();
}
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: StockBlockTradeVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: StockBlockTradeVO) => {
  reset();
  const _id = row?.id || ids.value[0]
  const res = await getStockBlockTrade(_id);
...
/** 提交按钮 */
const submitForm = () => {
  stockBlockTradeFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      form.value.assignId = Array.isArray(form.value.assignId)
        ...
        : form.value.assignId;
      if (form.value.id) {
        await updateStockBlockTrade(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addStockBlockTrade(form.value).finally(() =>  buttonLoading.value = false);
      }
...
/** 删除按钮操作 */
const handleDelete = async (row?: StockBlockTradeVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认删除大宗交易编号为"' + _ids + '"的数据项？※温馨提示：删除大宗交易会引起订单查询不到，如果是不再使用，请调整结束时间，或者禁用它。').finally(() => loading.value = false);
  await delStockBlockTrade(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('product/stockBlockTrade/export', {
    ...queryParams.value
  }, `stockBlockTrade_${new Date().getTime()}.xlsx`)
}

/** 锁仓按钮操作 */
const handleLock = async (row?: StockBlockTradeVO) => {
  const _ids = row?.id || ids.value;
  let str = _ids.join(",");
  await proxy?.$modal.confirm('是否确认锁仓大宗交易编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  const data = {
    ids:str,
    lockStatus:1
  }
  await lockStockBlock(data);
  proxy?.$modal.msgSuccess("锁仓成功");
  await getList();
}

/** 锁仓解锁按钮操作 */
const handleUnLock = async (row?: StockBlockTradeVO) => {
  const _ids = row?.id || ids.value;
  let str = _ids.join(",");
  await proxy?.$modal.confirm('是否确解仓大宗交易编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  const data = {
    ids:str,
    lockStatus:0
  }
