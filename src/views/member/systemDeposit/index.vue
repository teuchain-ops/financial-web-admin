    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['member:system:deposit:export']">导出</el-button>
          </el-col>
          <el-col :span="1.5">
...
import { systemDepositList } from '@/api/member/member';
import { SystemDepositQuery } from '@/api/member/member/types';
import { statisticsSystemDeposit } from "@/api/money/systemDeposit";
...
const getList = async () => {
  ...
  const [res, statistics] = await Promise.all([
    systemDepositList(queryParams.value),
    statisticsSystemDeposit(queryParams.value)
  ]);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('member/systemDepositExport', {
    ...queryParams.value
  }, `人工入款记录_${new Date().getTime()}.xlsx`)
}
