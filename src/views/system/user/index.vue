<template>
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <template #header>
            <el-row :gutter="10">
              ...
              <el-col :span="1.5">
                <el-button type="primary" plain @click="handleBindProxy()" :disabled="single" v-has-permi="['system:user:add']" icon="UserFilled">
                  绑定代理线
                </el-button>
              </el-col>
              <el-col :span="1.5">
          ...
          <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="代理线会员" align="center" prop="proxyMemberName" width="120" />
    ...
    <!-- 绑定代理线 -->
    <el-dialog title="绑定代理线" v-model="bindProxyDialog" width="500px" append-to-body>
      <el-form :model="form" :rules="rules" ref="userFormRef" label-width="120px">
        <el-form-item label="代理会员名称：" prop="proxyMemberName">
          <el-input v-model="form.proxyMemberName" placeholder="请输入代理会员名称" maxlength="30" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitBindProxy">确 定</el-button>
          <el-button @click="bindProxyDialog = false">取 消</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const data = reactive<PageData<UserForm, UserQuery>>({
  ...
  rules: {
    ...
    proxyMemberName:  [{ required: true, message: "代理会员名称不能为空", trigger: "blur" }],
  }
})
...
/** 绑定代理线按钮操作 */
const handleBindProxy = async (row?: UserForm) => {
  reset();
  const userId = row?.userId || ids.value[0]
  const { data } = await api.getUser(userId)
  Object.assign(form.value, data.user);
...
/** 提交代理线按钮操作 */
const submitBindProxy = () => {
  userFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      const data = {
        userId: form.value.userId,
        proxyMemberName: form.value.proxyMemberName,
      }
      await api.bindProxy(data);
